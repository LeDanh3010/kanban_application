...
class AuthController {
    ...
    if (!user) {
      res.clearCookie("refreshToken", { path: "/api/auth", httpOnly: true, secure: false, sameSite: "lax" });
    ...
    async refreshToken(req, res) {
        try {
            ...
            const newRefreshToken = jwt.sign({ id: user.id, jti: crypto.randomUUID() }, process.env.JWT_REFRESH_SECRET, { expiresIn: "30d" });
            ...
            try {
                ...
                return res.status(403).json({ error: "Token refresh failed, please login again" });
            }
